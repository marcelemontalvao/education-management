...
This frontend lives inside the `backend/frontend/` folder. Always run the full application from the parent `backend/` directory with `npm run dev`.
...
- npm or yarn
- PostgreSQL (v12 or higher) — required because the backend needs a database

### Setup and Run (required)
Frontend freelancers must run the backend as well. From the `backend/` folder:
...
Environment files are created automatically from `.env.example` on first setup. `npm run dev` starts both the API and the frontend. This is required to log in, load data, save forms, and test how the UI communicates with the backend.
...
If `frontend/.env` does not exist yet, it is created automatically from `frontend/.env.example` during `npm install` or `npm run dev`.
...
**Primary files**:
- `src/domains/notice/pages/add-notice-page.tsx`
- `src/domains/notice/components/notice-form.tsx`
- `src/domains/notice/api/notice-api.ts`
**Solution**: Check form field binding and API payload structure.

### Issue 2: Student CRUD Operations
**Problem**: Some CRUD operations for students may be incomplete on the backend.
**Related frontend files**:
- `src/domains/student/pages/add-student-page.tsx`
- `src/domains/student/pages/edit-student-page.tsx`
- `src/domains/student/api/student-api.ts`
...
For backend API documentation, see [../README.md](../README.md)
